...
const headersTableWidth = {
  ...
  SOLICITANTE : 150,
  MOTIVO : 80,
  "CANTIDAD SOLICITADA" : 70,
  "CANTIDAD UTILIZADA" : 70
}
...
const setMarginHeader = (text) => {
  const margin = 6;
  const abailables = ['FECHA', 'SOLICITANTE', 'MOTIVO'];
...
function preprocessTableData(data) {
  ...
  data.forEach((row, indexRow)=> {
      ...
      row.forEach((cell, indexCell) => {
          let newCell;
          if(indexRow===0){
            if(cell.toUpperCase() === 'CANTIDAD UTILIZADA'){
                result = true
            ...
            if(indexCell === resultIndex)
                resultValue += Number(cell.replaceAll(' ',''));
...
export const generateReportMaterialUse = async(data, start, end, material, unit) => {
        ...
        content: [
            {
                ...
                canvas: [
                    
                    {
                        ...
                        x1: 10, y1: 120, // Coordenadas de inicio
                        x2: 562, y2: 120, // Coordenadas finales
                    ...
                    w: 552, h: 120, // Ancho y alto del rectángulo
            ...
            },
            {
                ...
                {
                    text: `Registros de uso de material\n\n${material}\n\n${unit}\n\n${fechas}`,
                    color: 'black',
                    fontSize:12,
                    style: 'header',
                }
